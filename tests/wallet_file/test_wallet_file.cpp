...
// no password
// created by simplewallet from zurocoin-2.1.2, several tx received and sent
...
static void test_body(const zurocoin::Currency &currency, const std::string &path, const std::string &password,
    const std::vector<std::string> &addresses, bool view_only) {
	zurocoin::Wallet wallet("test_wallet_file.tmp", password);
	...
	for (auto &&a : addresses) {
		zurocoin::AccountPublicAddress address;
...
static void test_single_file(const zurocoin::Currency &currency, const std::string &path, const std::string &password,
    const std::vector<std::string> &addresses, bool view_only) {
	...
	test_body(currency, "test_wallet_file.tmp", password, addresses, view_only);
	{
		...
		if (si != zurocoin::Wallet::wallet_file_size(addresses.size()))
...
void test_wallet_file(const std::string &path_prefix) {
	zurocoin::Currency currency(false);
