...
static const char USAGE[] =
    R"(zurocoind )" zurocoin_VERSION_STRING R"(.
...
#endif
    R"(  --zurocoind-authorization=<usr:pass> HTTP authorization for RPC.)";
...
int main(int argc, const char *argv[]) try {
	...
	platform::ExclusiveLock coin_lock(coinFolder, "zurocoind.lock");

	logging::LoggerManager logManager;
	logManager.configure_default(config.get_data_folder("logs"), "zurocoind-");
...
//	block_chain.test_undo_everything();
	...
	std::cout << "zurocoind started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
