...
static void parse_peer_and_add_to_container(const std::string &str, std::vector<zurocoin::NetworkAddress> &container) {
	zurocoin::NetworkAddress na{};
...
const static UUID ZUROCOIN_NETWORK{{0x10, 0x02, 0x01, 0x02, 0x10, 0x09, 0x04, 0x04, 0x09, 0x11, 0x06, 0x07, 0x10, 0x05,
    0x11, 0x12}};  //
...
Config::Config(common::CommandLine &cmd)
    ...
    , network_id(ZUROCOIN_NETWORK)
    , p2p_bind_port(P2P_DEFAULT_PORT)
    , p2p_external_port(P2P_DEFAULT_PORT)
    , p2p_bind_ip("0.0.0.0")
    , zurocoind_bind_port(RPC_DEFAULT_PORT)
    , zurocoind_bind_ip("127.0.0.1")  // Less attack vectors from outside for ordinary uses
    , zurocoind_remote_port(0)
    , zurocoind_remote_ip("127.0.0.1")
	...
	if (const char *pa = cmd.get("--zurocoind-authorization")) {
		zurocoind_authorization = common::base64::encode(BinaryArray(pa, pa + strlen(pa)));
	}
	if (const char *pa = cmd.get("--zurocoind-bind-address")) {
		if (!common::parse_ip_address_and_port(pa, zurocoind_bind_ip, zurocoind_bind_port))
			throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
	}
	if (const char *pa = cmd.get("--zurocoind-remote-address")) {
		std::string addr         = pa;
		const std::string prefix = "https://";
		if (addr.find(prefix) == 0) {
#if !platform_USE_SSL
			throw std::runtime_error(
			    "Using https in --zurocoind-remote-address impossible - this binary is built without OpenSSL");
#endif
			std::string sip;
			std::string sport;
			...
			zurocoind_remote_port = boost::lexical_cast<uint16_t>(sport);
			zurocoind_remote_ip   = prefix + sip;
		} else {
			const std::string prefix2 = "http://";
			if (addr.find(prefix2) == 0)
				addr = addr.substr(prefix2.size());
			if (!common::parse_ip_address_and_port(addr, zurocoind_remote_ip, zurocoind_remote_port))
	...
	if (seed_nodes.empty() && !is_testnet)
		for (auto &&sn : zurocoin::SEED_NODES) {
			NetworkAddress addr;
