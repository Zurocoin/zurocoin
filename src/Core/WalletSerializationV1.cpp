...
/*struct WalletTransactionDto {
        ...
        WalletTransactionDto(const zurocoin::WalletTransaction &wallet) {
                state         = wallet.state;
        ...
        zurocoin::WalletTransactionState state;
        uint64_t timestamp;
        uint32_t block_height;
        Hash hash;
...
version(version) {}
        WalletTransferDto(const zurocoin::WalletTransfer &tr, uint32_t version)
...
/*void serialize(WalletRecordDto &v, zurocoin::ISerializer &s) {
...
std::string read_cipher(common::IInputStream &source, const std::string &name) {
	std::string cipher;
	//	zurocoin::BinaryInputStreamSerializer s(source);
...
std::string decrypt(const std::string &cipher, zurocoin::WalletSerializerV1::CryptoContext &crypto_ctx) {
	std::string plain;
...
void deserialize_encrypted(Object &obj, const std::string &name,
    zurocoin::WalletSerializerV1::CryptoContext &crypto_ctx, common::IInputStream &source) {
