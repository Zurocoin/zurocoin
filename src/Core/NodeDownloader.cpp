...
void Node::DownloaderV11::on_connect(P2PClientZurocoin *who) {
...
void Node::DownloaderV11::on_disconnect(P2PClientZurocoin *who) {
	if (who->is_incoming())
		return;
...
void Node::DownloaderV11::on_msg_notify_request_chain(P2PClientZurocoin *who,
...
void Node::DownloaderV11::advance_chain() {
	if (m_chain_client || !m_chain.empty())
		return;
	std::vector<P2PClientZurocoin *> lagging_clients;
	std::vector<P2PClientZurocoin *> sorted_clients;
	...
	std::sort(sorted_clients.begin(), sorted_clients.end(), [](P2PClientZurocoin *a, P2PClientZurocoin *b) -> bool {
...
void Node::DownloaderV11::on_msg_notify_request_objects(P2PClientZurocoin *who,
	...
	for (auto &&rb : req.blocks) {
		...
		try {
			...
			bid = zurocoin::get_block_hash(bheader);
...
void Node::DownloaderV11::advance_download(Hash last_downloaded_block) {
	...
	std::map<P2PClientZurocoin *, size_t> who_downloaded_counter;
	...
	for (auto &&dc : m_download_chain) {
		...
		if (total_downloading_blocks >= TOTAL_DOWNLOAD_BLOCKS)
			break;
		P2PClientZurocoin *ready_client = nullptr;
