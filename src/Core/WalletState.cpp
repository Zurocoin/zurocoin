...
void WalletPreparatorMulticore::thread_run() {
	while (true) {
		...
		api::zurocoind::SyncBlocks::SyncBlock sync_block;
...
void WalletPreparatorMulticore::cancel_work() {
	std::unique_lock<std::mutex> lock(mu);
	work = api::zurocoind::SyncBlocks::Response();
	prepared_blocks.clear();
	work_counter += 1;
}

void WalletPreparatorMulticore::start_work(const api::zurocoind::SyncBlocks::Response &new_work,
...
bool WalletState::sync_with_blockchain(api::zurocoind::SyncBlocks::Response &resp) {
...
bool WalletState::sync_with_blockchain(api::zurocoind::SyncMemPool::Response &resp) {
...
bool WalletState::redo_block(const api::BlockHeader &header, const PreparedWalletBlock &pb,
	...
	try {
		...
		std::exit(api::ZUROCOIND_DATABASE_ERROR);
	}
	return true;
}
