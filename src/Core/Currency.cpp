...
Currency::Currency(bool is_testnet)
	...
	std::string genesis_coinbase_tx_hex =
	    "010a01ff0001ffffffffffff07029b2e4c0281c0b02e7c53291a94d1d0cbff8883f8024f"
	    "5142ee494ffbbd088071210155d6d926b59097736229499adcf11be49ed2796d6b9e2360a8b8fc2894f41782";
...
uint32_t Currency::block_granted_full_reward_zone_by_block_version(uint8_t block_major_version) const {
	...
	if (block_major_version == 2)
		return zurocoin::parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V2;
	return zurocoin::parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1;
}
...
Hash zurocoin::get_transaction_inputs_hash(const TransactionPrefix &tx) {
	BinaryArray ba = seria::to_binary(tx.inputs);
	Hash new_hash  = crypto::cn_fast_hash(ba.data(), ba.size());
	return new_hash;
}

Hash zurocoin::get_transaction_prefix_hash(const TransactionPrefix &tx) {
...
Hash zurocoin::get_transaction_hash(const Transaction &tx) {
...
Hash zurocoin::get_block_hash(const BlockTemplate &bh) {
...
Hash zurocoin::get_auxiliary_block_header_hash(const BlockTemplate &bh) {
	return get_object_hash(get_block_hashing_binary_array(bh));
}

Hash zurocoin::get_block_long_hash(const BlockTemplate &bh, crypto::CryptoNightContext &crypto_ctx) {
