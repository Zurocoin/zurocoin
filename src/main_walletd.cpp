...
static const char USAGE[] =
    R"(walletd )" zurocoin_VERSION_STRING R"(.
...
Options:
  ...
  --data-folder=<full-path>            Folder for wallet cache, blockchain, logs and peer DB [default: )" platform_DEFAULT_DATA_FOLDER_PATH_PREFIX
    R"(zurocoin].
  --zurocoind-remote-address=<ip:port> Connect to remote zurocoind and suppress running built-in zurocoind.
  --zurocoind-authorization=<usr:pass> HTTP authorization for RCP.

Options for built-in zurocoind (run when no --zurocoind-remote-address specified):
  ...
  --zurocoind-bind-address=<ip:port>   Interface and port for zurocoind RPC [default: 127.0.0.1:8081].
...
static const bool separate_thread_for_zurocoind = true;
...
int main(int argc, const char *argv[]) try {
	...
	zurocoin::Config config(cmd);
	zurocoin::Currency currency(config.is_testnet);

	if (cmd.should_quit(USAGE, zurocoin::app_version()))
		return api::WALLETD_WRONG_ARGS;
	logging::LoggerManager logManagerNode;
	logManagerNode.configure_default(config.get_data_folder("logs"), "zurocoind-");
	...
	try {
		if (!config.zurocoind_remote_port)
			blockchain_lock = std::make_unique<platform::ExclusiveLock>(coinFolder, "zurocoind.lock");
	} catch (const platform::ExclusiveLock::FailedToLock &ex) {
		std::cout << "Zurocoind already running - " << ex.what() << std::endl;
		return api::ZUROCOIND_ALREADY_RUNNING;
	}
	try {
	...
	std::thread zurocoind_thread;
	if (!config.zurocoind_remote_port) {
		try {
			if (separate_thread_for_zurocoind) {
				zurocoind_thread = std::thread([&prm, &logManagerNode, &config, &currency] {
					boost::asio::io_service io;
			...
			std::cout << ex.what() << std::endl;
			if (zurocoind_thread.joinable())
				zurocoind_thread.join();  // otherwise terminate will be called in ~thread
			return api::ZUROCOIND_BIND_PORT_IN_USE;
		}
	}
